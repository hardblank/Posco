...
const isBankMovement=(item:unknown)=>{const value=item as {recordKind?:string;importInfo?:{documentType?:string}};return value.recordKind==="bank_movement"||value.importInfo?.documentType==="bank_statement"};
const dashboardExpenses=(value:unknown[])=>value.filter(item=>!isBankMovement(item));
const bankMovements=(value:unknown[])=>value.filter(isBankMovement);
...
export async function GET(req:Request){
  ...
  return Response.json({...globalData,monthKey:m.month_key,months,duePeriods:m.due_periods?.length?m.due_periods:[10],income:m.income,incomeRobson:m.income_robson,incomeGabi:m.income_gabi,freelancers:m.freelancers||[],foodRobson:m.food_robson,foodGabi:m.food_gabi,expenses:dashboardExpenses(m.expenses||[])});
}
