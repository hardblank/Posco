...
async function init(userId:string){
  ...
  await db("finance_months",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({user_id:userId,month_key:monthKey,income:0,income_robson:null,income_gabi:null,freelancers:[],food_robson:0,food_gabi:0,expenses:[],updated_at:now})});
}
...
export async function PUT(req:Request){
  ...
  const d=await req.json() as {monthKey:string;brand:string;heading:string;subtitle:string;themeBg:string;themeAccent:string;themeSurface:string;financeMode:"individual"|"family";primaryPersonName:string;secondaryPersonName:string;income:number;incomeRobson:number|null;incomeGabi:number|null;freelancers:unknown[];foodRobson:number;foodGabi:number;expenses:unknown[]};
  if(!validMonth(d.monthKey)||!["individual","family"].includes(d.financeMode)||!d.primaryPersonName?.trim()||(d.financeMode==="family"&&!d.secondaryPersonName?.trim())||!Number.isFinite(d.income)||!Array.isArray(d.expenses)||!Array.isArray(d.freelancers))return Response.json({error:"Dados inválidos"},{status:400});
  const now=new Date().toISOString(),uid=filter(userId);
  await Promise.all([db(`finance_state?user_id=eq.${uid}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({brand:d.brand,heading:d.heading,subtitle:d.subtitle,theme_bg:d.themeBg,theme_accent:d.themeAccent,theme_surface:d.themeSurface,finance_mode:d.financeMode,primary_person_name:d.primaryPersonName.trim(),secondary_person_name:d.secondaryPersonName.trim()||"Pessoa 2",updated_at:now})}),db(`finance_months?user_id=eq.${uid}&month_key=eq.${filter(d.monthKey)}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({income:d.income,income_robson:d.incomeRobson,income_gabi:d.incomeGabi,freelancers:d.freelancers,food_robson:d.foodRobson,food_gabi:d.foodGabi,expenses:d.expenses,updated_at:now})})]);
