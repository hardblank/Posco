...
export async function POST(req:Request){
 ...
 const knownExpenses=allRows.flatMap(row=>row.expenses||[]),parsed=parseStatement(payload.content,payload.fileName,month.expenses||[],payload.documentType,knownExpenses,payload.targetMonth),expenses=parsed.items.filter(item=>item.direction==="expense");if(!expenses.length)return Response.json({error:"Nenhuma saída válida foi encontrada. O arquivo precisa mostrar data, descrição e valor; no PDF, o texto deve ser selecionável."},{status:422});
