...
export async function POST(req:Request){
 ...
 const parsed=parseStatement(payload.content,payload.fileName,month.expenses||[],payload.documentType),expenses=parsed.items.filter(item=>item.direction==="expense");if(!expenses.length)return Response.json({error:"Nenhuma saída bancária válida foi encontrada. Prefira um arquivo OFX ou CSV com data, descrição e valor."},{status:422});
 ...
 for(const transaction of expenses){
  ...
  newExpenses.push({id:Date.now()+newExpenses.length,name:transaction.description,rawDescription:transaction.rawDescription,paymentMethod:transaction.paymentMethod,amount:transaction.amount,type:isMovement?"Movimentação":"Variável",recordKind:isMovement?"bank_movement":"expense",category,categoryConfidence:transaction.confidence,owner,note:`Importado de ${accountName} • ${fileName}`,confirmed:transaction.duplicateKind?false:true,period:duePeriodFor(transaction.date,month.due_periods||[10]),paymentStatus:isMovement?"paid":"pending",paidDate:isMovement?transaction.date:"",transactionDate:transaction.date,source:"bank_import",importInfo:{fingerprint:transaction.fingerprint,accountName,fileName,documentType:payload.documentType,importedAt:now},possibleDuplicateOf:transaction.matchedExpenseId});
 }
