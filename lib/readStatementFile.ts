...
async function readPdfOnServer(file:File){if(file.size>3_000_000)throw new Error("Este PDF é muito grande para a leitura alternativa do celular. Envie um PDF de até 3 MB ou use o arquivo OFX/CSV.");const response=await fetch("/api/pdf-text",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({fileName:file.name,data:await toBase64(file)})}),result=await response.json() as {text?:string;error?:string};if(!response.ok||!result.text)throw new Error(result.error||"Não foi possível ler este PDF.");return result.text}
